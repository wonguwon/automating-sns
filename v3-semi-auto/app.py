...
------------------------------------------------
6단계(후보 가져오기 → 딥리서치 → 콘텐츠 JSON → 표지 이미지 → 콘텐츠 렌더 → 릴스 렌더)를
상단 네비게이션의 별도 페이지로 분리했다. 각 페이지는 data/pipeline/<content_id>.json
매니페스트와 각 단계의 canonical 파일(data/candidates, data/research, data/content,
카드뉴스/<id>/)을 디스크에서 직접 읽고 쓰므로, 이전 단계를 같은 세션에서 이어서 실행하지
않아도 원하는 단계부터 바로 진행할 수 있다.
...
pages = [
    ...
    st.Page("pages/6_릴스_렌더.py", title="6. 릴스 렌더"),
]
